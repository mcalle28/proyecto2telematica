/**
 * Clase lector
 * lee y transfiere los archivos bmps a vectores o arreglos que puedan ser
 * leidos por las demas clases
 * */
...
/*Estructura bmp para los archivos*/
typedef struct
{
...
class lector
{
  ...
  public:
    ...
    /*Crea los macrobloques para la primera imagen*/
    ...
    /*Retorna los arreglos de macrobloques de cada imagen en un vector*/
    ...
    /*Crea un arreglo de macrobloques para la segunda imagen*/
    vector<macrobloque> crearArregloMBpxp(vector<vector<int> > pixelMatrix, int w, int h)
    {
       ...
       /*llena una matriz de pixeles para luego crear los macrobloques*/
    vector<vector<int> > llenarMatrizPixeles(char *filename){
        ...
        queue<int> matrizCopia;

        for (int i = w - 1; i >= 0; --i){
            for (int j = 0; j < h; ++j){
                matrizCopia.push((int)frame1bmp.pixels[i][j]);
            }
        }
        ...
        for (int i = 0; i < w; i++){
            ...
            for (int j = 0; j < h; j++){
                matrizPixeles[i].push_back(matrizCopia.front());
                matrizCopia.pop();
            }
        }
        ...
        if (bmp_file != NULL){
            ...
            if (fread((void *)bmp_header, sizeof(unsigned char),
                ...
                fprintf(stderr, "Error, asegurese de tener el formato correcto %s", file_name);
                exit(EXIT_FAILURE);
            }
